...
resource "aws_security_group" "web_sg" {
  # nameとdescriptionは指定。空欄だとよくわからない文字列が入る。
  name        = "${var.project}-${var.environment}-web-sg"
  description = "web front role security group"
  # 同一ディレクトリにtfファイルがある場合、vpcも参照可能。
...
resource "aws_security_group_rule" "web_in_http" {
  ...
  type              = "ingress" # インバウンド
  # ここは、tcp,udpなどのトランスポート層やネットワーク層のプロトコルを記載。
  # httpとかsshはNG。
  protocol    = "tcp"
  from_port   = 80 # httpのポート
  to_port     = 80
...
resource "aws_security_group_rule" "web_out_tcp3000" {
  ...
  type                     = "egress" # アウトバウンド
  protocol                 = "tcp"
  from_port                = 3000 # node.js用ポート
  to_port                  = 3000
  # appへ出るため、["0.0.0.0/0"]はNG。
...
resource "aws_security_group_rule" "app_out_http" {
  ...
  from_port         = 80
  to_port           = 80
  # 外部への接続は「source_security_group_id」ではなくプレフィックスリストを使用
  # S3へ接続するため以下の通り記載。
...
# operation and management security group（運用管理）
...
resource "aws_security_group_rule" "db_in_tcp3000" {
  ...
  to_port                  = 3306
  source_security_group_id = aws_security_group.app_sg.id # appから入る
}
